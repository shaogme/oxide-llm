...
function createMockServer(port) {
    ...
    mock.on({ userMessage: /weather in Tokyo/i }, {
        toolCalls: [
            {
                name: 'get_weather',
    ...
    mock.on({ userMessage: /weather in Berlin/i }, {
        toolCalls: [
            {
                name: 'get_weather',
    ...
    mock.on({ userMessage: /weather in London/i }, {
        toolCalls: [
            {
                name: 'get_weather',
    ...
    mock.on({ userMessage: /weather in Paris/i }, {
        toolCalls: [
            {
                name: 'get_weather',
    ...
    mock.on({ userMessage: /weather in Shanghai/i }, {
        toolCalls: [
            {
                name: 'get_weather',
    ...
    mock.on({ userMessage: /Multi-turn weather in Beijing/i, hasToolResult: false }, {
        toolCalls: [
            {
                name: 'get_weather',
    ...
    mock.on({ hasToolResult: true }, {
        content: 'The weather in Beijing is 20°C with sunny skies.',
    });
    // 3. System prompt match
    mock.on({ systemMessage: /you are a helpful assistant with system prompt/i }, {
    ...
    mock.on({ userMessage: /Trigger 401 Unauthorized/i }, {
        status: 401,
        error: {
    ...
    mock.on({ userMessage: /Trigger 500 Internal Error/i }, {
        status: 500,
        error: {
